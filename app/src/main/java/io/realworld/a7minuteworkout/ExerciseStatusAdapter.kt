...
class ExerciseStatusAdapter(val items:ArrayList<ExerciseModel>,val context:Context) :RecyclerView.Adapter<ExerciseStatusAdapter.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {     //when created do this
        return ViewHolder(LayoutInflater.from(context).
        inflate(R.layout.item_exercise_status,parent,false))        //inflate => display this layout
    }

    override fun onBindViewHolder(holder: ViewHolder, position: Int) {      //what each view should contain
    ...
    override fun getItemCount(): Int {      //total items
        return items.size
    }
}